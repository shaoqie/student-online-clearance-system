<div id="socs-carousel" class="carousel slide">
    <div class="carousel-inner">
        <div class="item active">
            <div class="container">
                <img src="{$host}/public/img/carousel/1.jpg" alt="" style="width: 960px; height: 400px;"/>
                <div id="welcome" class="carousel-caption socs-welcome hero-unit">
                    <h1>Welcome to SOCS!</h1>
                    <p class="lead">The Student Online Clearance System of The University of Southeastern Philippines, Davao City.</p>
                    <button class="btn btn-primary btn-large">Learn More &raquo;</button><br>
                </div>
            </div>
        </div>
        <div class="item">
            <div class="container">
                <img src="{$host}/public/img/carousel/2.jpg" alt="" style="width: 960px; height: 400px;"/>
                <div id="welcome" class="carousel-caption socs-welcome hero-unit">
                    <h1>Student of USEP?</h1>
                    <p class="lead">Register with us! to have your clearance on the go.</p>
                    <button class="btn btn-primary btn-large">Learn More &raquo;</button><br>
                </div>
            </div>
        </div>
        <div class="item">
            <div class="container">
                <img src="{$host}/public/img/carousel/3.jpg" alt="" style="width: 960px; height: 400px;"/>
                <div id="welcome" class="carousel-caption socs-welcome hero-unit">
                    <h1>Assigned as a Signatory-in-charge?</h1>
                    <p class="lead">Register here</p>
                    <button class="btn btn-primary btn-large">Learn More &raquo;</button><br>
                </div>
            </div>
        </div>
    </div>
    <a class="carousel-control left" href="#socs-carousel" data-slide="prev">&lsaquo;</a>
    <a class="carousel-control right" href="#socs-carousel" data-slide="next">&rsaquo;</a>
</div>

<!--
<div id="welcome" class="hero-unit">

    <h1>Welcome to SOCS!</h1>
    <p>The Student Online Clearance System of The University of Southeastern Philippines, Davao City.</p>
    <button class="btn btn-primary btn-large">Learn More &raquo;</button><br>
</div>-->